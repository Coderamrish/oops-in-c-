...
#include <algorithm>
using namespace std;
int calculateMinimumTime(vector<int>& firstStartTimes,
                         vector<int>& firstDurations,
                         vector<int>& secondStartTimes,
                         vector<int>& secondDurations)
{
    ...
    for(int i = 0; i < firstStartTimes.size(); i++)
    {
        ...
        earliestFirstCompletion =
            min(earliestFirstCompletion, completionTime);
    }
    int minimumTotalTime = INT_MAX;
    for(int i = 0; i < secondStartTimes.size(); i++)
    {
        int secondActivityStart =
            max(earliestFirstCompletion,
                secondStartTimes[i]);
        int totalTime =
            secondActivityStart +
            secondDurations[i];
        minimumTotalTime =
            min(minimumTotalTime, totalTime);
    }
